...
'''
# Print a grid (3x3) with all mathematical values from 0-8 set to 0
l=[0]*9
...
def display():
    for i in range(len(l))
        if i%3==0
            print()
        print()
# 'Players' Can change the value on each square, IF the value of the square is 0 (unoccupied)

# Game ends on 2 conditions: Initially if values on grid = (winner) ELSE if all values on grid are occupied with NO winner (draw)
def CheckWinner(sym):
    if ((l[8])):
        return True
    else:
